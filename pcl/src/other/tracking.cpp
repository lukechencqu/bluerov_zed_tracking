...
/*
  ...
  pcl::toROSMsg(*downsampledCloud, downsampledCloudMsg);

  downsampledcloud_pub.publish(downsampledCloudMsg);
*/
}
...
main (int argc, char** argv)
{
  ...
  // ros::Subscriber cloud_sub = node.subscribe("/zed/zed_node/point_cloud/cloud_registered", 30, cloudCB);  
  ros::Subscriber cloud_sub = node.subscribe("/bluerov2h_follower/depth/points2", 10, cloudCB);  
  ...
  ROS_INFO_STREAM("compute3DCentroid: c[0]=" << c[0] << " c[1]=" << c[1] << " c[2]=" << c[2]);
